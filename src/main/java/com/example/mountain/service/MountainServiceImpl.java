...
public class MountainServiceImpl implements MountainService {
    ...
    public Mono<MountainPageResp> getMountainListPage(int district, int pageNum, int pageSize, String sortBy, boolean isAsc, String search) {
        ...
        if (district == 0) {
            mountainRepository.findSize(search).doOnNext(x -> mountainPageResp.setTotalPageSize(x.longValue())).retry().subscribe();
            ...
            mountainRepository.findSize(district, search).doOnNext(x -> mountainPageResp.setTotalPageSize(x.longValue())).retry().subscribe();
