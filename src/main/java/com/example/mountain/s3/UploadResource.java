...
//    @PostMapping
//    public Mono<ResponseEntity<UploadResult>> uploadHandler2(@RequestHeader HttpHeaders headers, @RequestBody Flux<ByteBuffer> body) {
//
...
//    }
    ...
    @PostMapping
    public Mono<ResponseEntity<UploadResult>> uploadHandler(@RequestHeader HttpHeaders headers, @RequestBody Mono<Part> body) {
        long length = headers.getContentLength();
        if (length < 0) {
...
//        body.doOnNext(fp -> System.out.println("Received File : " + fp.filename()))
...
//                .putObject(PutObjectRequest.builder()
//                                .bucket(s3config.getBucket())
//                                .contentLength(body.flatMap(x-> x.headers().get("length")))
//                                .key(fileKey.toString()+".jpg")
//                                .contentType(MediaType.APPLICATION_OCTET_STREAM_VALUE)
//                                .metadata(metadata)
//                                .build(),
//                        AsyncRequestBody.fromPublisher(body.map(x->x.))));
